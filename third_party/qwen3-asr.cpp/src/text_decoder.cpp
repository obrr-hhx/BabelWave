...
bool TextDecoder::init_kv_cache(int32_t n_ctx) {
    ...
    // Live transcription repeatedly decodes progressively longer snapshots of
    // the same utterance. Reallocating the Metal KV buffer for every snapshot
    // adds latency and memory churn. Reuse an existing sufficiently large
    // cache; stale cells are ignored once n_used is reset.
    if (state_.cache.buffer && state_.cache.ctx &&
    ...
    // Allocate in coarse blocks so several progressive snapshots fit before
    // the next growth. A 512-token cache covers normal live-caption segments.
    constexpr int32_t kCacheBlock = 256;
    constexpr int32_t kMinimumCache = 512;
    const int32_t requested = std::max(n_ctx, kMinimumCache);
    const int32_t capacity = ((requested + kCacheBlock - 1) / kCacheBlock) * kCacheBlock;

    state_.cache.n_ctx = capacity;
    ...
    for (int il = 0; il < cfg.n_decoder_layers; ++il) {
        state_.cache.k_cache[il] = ggml_new_tensor_3d(
            ...
            cfg.head_dim, cfg.n_key_value_heads, capacity);
...
bool TextDecoder::load_vocab(gguf_context * ctx) {
    ...
    for (int32_t token = 0; token < static_cast<int32_t>(vocab_.size()); ++token) {
        if (vocab_[token] == "<asr_text>") {
            prompt_token_ids_["<asr_text>"] = token;
        }
        const std::string piece = decode_token(token);
        if (piece == "language" || piece == " Chinese" || piece == " English" ||
            piece == " Japanese" || piece == " Korean") {
            prompt_token_ids_[piece] = token;
        }
    }
...
std::vector<int32_t> TextDecoder::forced_language_prompt_tokens(
    const std::string & language) const {
    const auto language_token = prompt_token_ids_.find("language");
    const auto language_name_token = prompt_token_ids_.find(" " + language);
    const auto asr_text_token = prompt_token_ids_.find("<asr_text>");
    if (language_token == prompt_token_ids_.end() ||
        language_name_token == prompt_token_ids_.end() ||
        asr_text_token == prompt_token_ids_.end()) {
        return {};
    }
    return {language_token->second, language_name_token->second, asr_text_token->second};
}
