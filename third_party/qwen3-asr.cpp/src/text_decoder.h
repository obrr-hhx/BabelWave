...
public:
    ...
    // Tokenize the fixed official Qwen3-ASR forced-language prefix:
    // "language {Language}<asr_text>". The model emits these same first two
    // text pieces in auto-detection mode, so each component is represented by
    // one vocabulary token in the converted GGUF.
    [[nodiscard]] std::vector<int32_t> forced_language_prompt_tokens(
...
private:
    ...
    std::map<std::string, int32_t> prompt_token_ids_;
    int n_threads_ = 4;
};
