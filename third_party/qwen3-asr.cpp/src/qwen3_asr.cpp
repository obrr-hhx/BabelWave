...
transcribe_result Qwen3ASR::transcribe_internal(const float * samples, int n_samples,
    ...
    const bool forced_language = !params.language.empty();
    const size_t text_start = forced_language
        ? 0
        : std::min<size_t>(output_tokens.size(), 2);  // remove auto-detected language prefix
...
std::vector<int32_t> Qwen3ASR::build_input_tokens(int32_t n_audio_frames,
    ...
    if (!language.empty()) {
        const std::vector<int32_t> forced_prefix =
            decoder_.forced_language_prompt_tokens(language);
        if (forced_prefix.empty()) {
            error_msg_ = "Could not tokenize forced ASR language: " + language;
            return {};
        }
        tokens.insert(tokens.end(), forced_prefix.begin(), forced_prefix.end());
    }
...
bool Qwen3ASR::decode_greedy(const std::vector<int32_t> & input_tokens,
    ...
    if (input_tokens.empty()) {
        if (error_msg_.empty()) error_msg_ = "ASR input prompt is empty";
        return false;
    }
